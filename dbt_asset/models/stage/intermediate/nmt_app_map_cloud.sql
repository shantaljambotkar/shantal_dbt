With AssetMapAWS AS (
	...
	from {{ref('src_cm__aws')}} ssi 
	where uai is not null)inv
	inner join {{ref('src_opal__app')}} voa on trim(lower(inv.uai)) = trim(lower(voa.u_unique_app_id))
),
AssetMapAzure AS (
	select asset_id, voa.app_id as app_asset_id from (
	select asset_id, unnest(regexp_matches(uai, '[uU][aA][iI]\d+', 'g')) uai
	from {{ref('src_cm__azure')}} ssi 
	where uai is not null)inv
	inner join {{ref('src_opal__app')}} voa on trim(lower(inv.uai)) = trim(lower(voa.u_unique_app_id))
),
AssetMapGEIX AS (
